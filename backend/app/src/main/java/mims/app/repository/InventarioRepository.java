...
public interface InventarioRepository extends JpaRepository<InventarioEntity, Integer> {
        ...
        @Query(value = "SELECT inventario.id, inventario.cantidad, inventario.precio, joya.nombre AS joya, tipo_joya.nombre AS tipo_joya, locacion.nombre AS local FROM mims.inventario, mims.joya, mims.locacion, mims.tipo_joya WHERE inventario.id_joya = joya.id AND inventario.id_tipo_joya = tipo_joya.id AND inventario.id_locacion = locacion.id", nativeQuery = true)
