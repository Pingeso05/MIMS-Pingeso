...
public interface LogInventarioRepository extends JpaRepository<LogInventarioEntity, Integer> {
    @Query(value = "SELECT log_inventario.id, log_inventario.id_producto, joya.nombre AS nombre_producto " +
            "FROM mims.log_inventario, mims.inventario, mims.joya " +
            "WHERE log_inventario.id_producto = inventario.id AND inventario.id_joya = joya.id", nativeQuery = true)
